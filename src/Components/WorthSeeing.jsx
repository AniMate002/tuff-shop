...
export default function WorthSeeing({products, amount, error, isLoading}){
    ...
    const renderedProducts = trending.map(product => {
        return(
            <Link to={`/products/${product.id}`} key={product.id} className="rounded-b-lg">
                <img src={product.images[0]} className=" rounded-t-lg laptop:h-[300px] h-[250px] mx-auto"/>
                <div className="px-4 py-4">
                    <h1 className="text-md font-bold text-white mb-2 text-center">{product.title}</h1>
                </div>
            </Link>
        )
    })
    ...
    return(
        <div className=" bg-secondaryDarkColor px-10 py-7 rounded-lg mt-10 ">
            <h1 className="text-center text-white font-bold text-2xl mb-7">Worth Seeing</h1>
